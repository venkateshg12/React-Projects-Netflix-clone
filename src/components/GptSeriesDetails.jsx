...
const GptSeriesDetails = () => {
  const store = useSelector(store => store?.info?.seriesInfo);
  const category = useSelector(store => store?.info?.seriesCategory);
        ...
        <div className='relative items-center place-items-center p-5  grid max-[1280px]:grid-cols-2 gap-4 min-[1280px]:grid-cols-4  max-[650px]:grid-cols-1 z-50 border-4 border-violet-950 rounded-2xl'>
          {store.map(store => (
            <div className='border-3 border-violet-700 p-4 md:p-9 rounded-2xl'>
              <MovieCard movie={store} type="tv" className="lg:h-[25rem] max-w-[25rem] md:w-auto max-[1270px]:h-[20rem] " />
            </div>
          ))}
        </div>
      </div>
...
export default GptSeriesDetails;